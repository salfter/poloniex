...
class poloniex:
    ...
    def api_query(self, command, req={}):
        ...
        if(command == "returnTicker" or command == "return24Volume" or command == "returnCurrencies"):
            ...
            url='http://poloniex.com/public?command=returnOrderBook&currencyPair=' + str(req['currencyPair'])
            if "depth" in req:
                url += '&depth=' + str(req['depth'])
            ...
            url='http://poloniex.com/public?command=returnTradeHistory&currencyPair=' + str(req['currencyPair'])
            if "start" in req:
                url+='&start=' + str(req['start'])
            if "end" in req:
                url+='&end=' + str(req['end'])
            ret = urllib2.urlopen(urllib2.Request(url))
            return json.loads(ret.read())
